...
var playbackWriter = new FileWriter('audio/playback.wav', {
  sampleRate: 24000,
  channels: 1
});

const inputFiles = [
  `${path.resolve('./audio/broadcast.wav')}`,
  `${path.resolve('./audio/response.wav')}`
];
...
var self = module.exports = {
  ...
  sendAudioInput: function () {
    let raw = []
    const assistant = self.setUser('dependentsassistant');
    fs.readFile(`${path.resolve('./audio/response.wav')}`, (err, file) => {
  ...
  outputFileStream: function () {
    return new FileWriter(path.resolve('./audio/response.wav'), {
